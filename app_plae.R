...
plae_heatmap <- readRDS("./data/plae_centroid_hv_plot.Rds")
...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("PLAE Comparison", tabName = 'PLAE', icon = icon('play', lib = 'font-awesome')),
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName='PLAE',
            h2("PLAE Comparison"),
            fluidPage(
              fluidRow(column(12,plotOutput("PLAE_heatmap"), style = 'padding:4px;')),
              fluidRow(selectInput("Cluster_plae", label = 'PLAE Centroid', choices = NULL)),
              fluidRow(column(6,plotOutput("PLAE_dimplot"), style = 'padding:4px;'),
                       column(6, plotOutput("PLAE_violin"), style = 'padding:4px;'))
            )),
...
server <- function(input, output,session) {
  ...
  md_for_coexpression <- eventReactive(input$LoadCoGene,{
    md <- obj@meta.data
  ...
  output$Scatter_CoGene <- renderPlot({
    ...
    scatter_plot <- ggplot(md_for_coexpression(), aes(x = gene1, y = gene2, color = seurat_clusters)) +
      geom_point() +theme_bw() +
  ...
  output$FeatPlot_CoGene_CT <- renderPlot({
    ...
    FeaturePlot(obj, features = c(CoGene1(), CoGene2()), blend = T, pt.size = 0.1, cols = c('lightgrey','red','green'))
  })
  ...
  output$Scatter_CoGene_CT <- renderPlot({
    ...
    req(CoGene2() %in% rownames(obj))
    req(input$LoadCoGene)
    req(Cluster_coGene() %in% md_for_coexpression()$seurat_clusters)
    
    scatter_plot <- ggplot(md_for_coexpression()[md_for_coexpression()$seurat_clusters == Cluster_coGene(),],
                           aes(x = gene1, y = gene2,
  ...
  clusters.to.select_plae <- colnames(obj@meta.data)[grepl('^plae:',colnames(obj@meta.data))]
  
  updateSelectizeInput(session,'Cluster_plae', choices = clusters.to.select_plae, server = T)
  
  Cluster_plae <- reactive((input$Cluster_plae))
  
  output$PLAE_heatmap <- renderPlot({
    plae_heatmap$data$Var2 <- substr(plae_heatmap$data$Var2,1,35)
    plae_heatmap
  })
  
  output$PLAE_dimplot <- renderPlot({
    obj$plae_dimplot_label <- tstrsplit(obj$plae_correlation_bothHV,'__', keep = 2)[[1]]
    DimPlot(obj, group.by = 'plae_dimplot_label', label = T) +
      theme(legend.position = 'bottom') +
      ggtitle('Max Corr. Cluster Labels')
  })
  
  output$PLAE_violin <- renderPlot({
    req(Cluster_plae() %in% colnames(obj@meta.data))
    # Violin
    VlnPlot(obj, features = Cluster_plae(), pt.size = 0) + NoLegend()
  })
