...
```{r}
plae_mouse <- plae_mouse[,!grepl('Doublet', colnames(plae_mouse))]
```
...
obj$plae_correlation_all <- factor(obj$seurat_clusters, labels = colnames(cor_matrix)[apply(cor_matrix,1,which.max)])
...
obj$plae_correlation_ourHV <- factor(obj$seurat_clusters, labels = colnames(cor_matrix)[apply(cor_matrix,1,which.max)])
...
ggplot(temp, aes(x = Var1, y = Var2, fill = value)) +
  ...
  xlab('Clusters') +
  ylab('PLAE Centroids')
plae_centroid_hv <- ggplot(temp, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(low = 'blue', mid = 'white', high = 'red', midpoint = mean(temp$value)) +
  geom_point(data = temp[temp$top3,],
             aes(x = Var1, y = Var2), shape = 8) +
...
obj$plae_correlation_theirHV <- factor(obj$seurat_clusters, labels = colnames(cor_matrix)[apply(cor_matrix,1,which.max)])
...
obj$plae_correlation_bothHV <- factor(obj$seurat_clusters, labels = colnames(cor_matrix)[apply(cor_matrix,1,which.max)])
...
```{r}
# Using our HVGs overlapped with all PLAE 
gene_set <- intersect(rownames(plae_mouse), intersect(rownames(obj), VariableFeatures(obj)))
...
temp_cor <- cor(as.matrix(obj@assays$RNA$data[gene_set,]), # need to convert to a matrix over sparse matrix
                ...
                method = 'spearman')
# getwd()
saveRDS(temp_cor,'./data/plae_correlation_matrix.Rds')

colnames(temp_cor) %in% colnames(obj@meta.data)

# table(rownames(temp_cor) == colnames(obj))

obj@meta.data[paste0('plae:', colnames(temp_cor))] <- temp_cor
```
...
saveRDS(plae_centroid_hv, './data/plae_centroid_hv_plot.Rds')
```
